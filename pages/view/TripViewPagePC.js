...
export default function TripViewPagePC() {
  ...
  const refundInfo = useMemo(() => {
    ...
    const lines = [];
    if (typeof fullH === 'number') lines.push(`Полный возврат: до ${fullH} ч до начала поездки — 100%.`);
    if (typeof partPct === 'number' && typeof partH === 'number') lines.push(`Частичный возврат: ${partPct}% не позднее чем за ${partH} ч до начала поездки.`);

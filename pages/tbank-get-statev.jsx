...
export default function TbankGetStatePage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const r = await fetch('/api/tbank/get-statev', {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 920, margin: '40px auto', padding: '0 16px', fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif' }}>
      <h1 style={{ marginBottom: 8 }}>T-Банк: GetState (E2C v2 — выплаты)</h1>
      <p style={{ color: '#666', marginTop: 0 }}>
        Диагностическая страница статуса <code>PaymentId</code> по API выплат <b>E2C v2</b>.
      </p>
...
{JSON.stringify(resp.debug, null, 2)}
      ...
      <footer style={{ marginTop: 28, fontSize: 12, color: '#6b7280' }}>
        Используются переменные: <code>TBANK_TERMINAL_KEY</code> и <code>TBANK_SECRET</code>.
        Терминал формируется как <code>{'<TBANK_TERMINAL_KEY>'} + 'E2C'</code> (если суффикса нет).
      </footer>
    </div>
  );
}

...
// Мобильная страница редактирования поездки.
// По UI/стилям — копия CreateTripMobile, но логика берётся из useEditTrip (как в EditTripPC).

import dynamic from 'next/dynamic';
import { useRouter } from 'next/router';
import styles from '../../styles/create-trip.mobile.module.css';
import { useEditTrip } from '../../lib/useEditTrip';
...
export default function EditTripMobile({ tripId, mainContentRef: externalMainContentRef }) {
  const router = useRouter();

  const tripIdFromQuery = typeof router?.query?.tripId === 'string' ? router.query.tripId : null;
  const effectiveTripId = tripId || tripIdFromQuery;

  // Если страницу рендерим как отдельный роут — нам нужен свой скролл-контейнер.
  // Если же компонент будет когда-то встроен в DashboardMobile — можно передать mainContentRef извне.
  const internalMainContentRef = useRef(null);
  const mainContentRef = externalMainContentRef || internalMainContentRef;
  const shouldRenderOwnScrollContainer = !externalMainContentRef;

  const handleBackToParticipants = () => {
    const id = effectiveTripId;
    if (!id) {
      router.back();
      return;
    }
    ...
    router.push(
      {
        pathname: '/dashboard',
        query: { section: 'participants', tripId: id },
      },
      undefined,
      { shallow: true }
    );
  ...
  // Сохраняем текущий scrollTop, чтобы при открытии/закрытии карты не «прыгал» экран.
  useEffect(() => {
    ...
    const updatePopupPosition = () => {
      ...
      const errorPopupHeight = 100; // приблизительно
    ...
    el.addEventListener('scroll', updatePopupPosition);
    updatePopupPosition();

    if (!isLocationFromOpen && !isLocationToOpen) {
      requestAnimationFrame(() => {
        if (mainContentRef?.current) mainContentRef.current.scrollTop = scrollPositionRef.current;
      });
    }
    ...
    return () => {
      if (el) {
        ...
        el.removeEventListener('scroll', updatePopupPosition);
      }
    };
  }, [isLocationFromOpen, isLocationToOpen, refundError, timeError, showPopup, mainContentRef]);

  // Подгружаем ymaps, чтобы apply (reverse geocode) работал стабильно.
  useEffect(() => {
    const loadYmaps = async () => {
      ...
      window.ymaps.ready(() => setIsYmapsLoaded(true));
    };
    loadYmaps();
  }, []);
  ...
  const handleCloseMap = () => {
    if (isLocationFromOpen) closeLocationFrom();
    else if (isLocationToOpen) closeLocationTo();
  };
  ...
  const isMapVisible = Boolean(isLocationFromOpen || isLocationToOpen);
  ...
  // блокирующий оверлей — только если нет ни карты, ни подходящей компании
  const mustBindCard = useMemo(() => !hasValidCard && !hasCompanyOk, [hasValidCard, hasCompanyOk]);
  ...
  const content = (
    <div className={styles.animatedWrapper}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        {/* В create-trip.mobile.module.css может не быть отдельного back-класса,
            поэтому используем submitButton + лёгкий inline-тюнинг */}
<button
  type="button"
...
>
  ← Назад
      ...
      <h1 className={styles.header}>Редактировать поездку</h1>
      ...
      <div className={styles.formWrapper} style={{ display: isMapVisible ? 'none' : 'block' }}>
        <form onSubmit={showTimezoneInput ? handleTimezoneSubmit : handleSubmit} className={styles.form}>
            ...
            <div className={styles.inputGroup}>
              ...
              <div className={styles.switchContainer}>
                <label className={styles.switch}>
                  <input
                    ...
                    onChange={(e) =>
                      handleChange({
                        target: { name: 'isCompanyTrip', value: e.target.checked ? 'true' : 'false' },
                      })
                    }
                    disabled={loading}
                  />
              ...
              {!hasValidCard && hasCompanyOk && (
                <p className={styles.hint}>
                  У вас нет привязанной карты — по умолчанию поездка сохраняется от лица компании.
          ...
          <div className={styles.section}>
            ...
            <div className={styles.inputGroup}>
              <label>
                Цена <span className={styles.ruble}>₽</span>
              </label>
              <div className={styles.inputWrapper}>
                ...
                {Number(tripData.price) > 0 && (
                  <div className={styles.feeCalculation}>
                    <h3>Расчёт выплат</h3>
                    <p>
                      Цена поездки: {tripData.price} ₽ <span className={styles.hint}>(сумма, которую заплатят участники)</span>
                    </p>
                    <p>
                      Комиссия площадки ({platformSettings.platformFeePercent}%): {tripData.platformFee.toFixed(2)} ₽
                    </p>
                    <p>
                      Комиссия Т-Банка ({platformSettings.tbankFeePercent}%): {tripData.tbankFee.toFixed(2)} ₽
                    </p>
            ...
            <div className={`${styles.inputGroup} ${styles.ageRange}`}>
              ...
              <div>
                <input type="number" name="ageFrom" value={tripData.ageFrom} onChange={handleChange} placeholder="От" required disabled={loading} />
                <input type="number" name="ageTo" value={tripData.ageTo} onChange={handleChange} placeholder="До" required disabled={loading} />
              </div>
            </div>
              ...
              <div className={styles.switchContainer}>
                <label className={styles.switch}>
                  <input
                    ...
                    onChange={(e) =>
                      handleChange({
                        target: { name: 'alcoholAllowed', value: e.target.checked ? 'true' : 'false' },
                      })
                    }
                    disabled={loading}
                  />
            ...
            <div className={styles.inputGroup}>
              ...
              <div className={styles.switchContainer}>
                <label className={styles.switch}>
                  <input
                    ...
                    onChange={(e) =>
                      handleChange({
                        target: { name: 'refund_policy_type', value: e.target.checked ? 'true' : 'false' },
                      })
                    }
                    disabled={loading}
                  />
              ...
              <p className={styles.hint}>
                Выберите, чтобы задать свои сроки и проценты возврата денег при отмене поездки участниками.
              </p>
          ...
          <div className={styles.section}>
            ...
            <div className={`${styles.inputGroup} ${styles.locationGroup}`}>
              ...
              <div className={styles.locationInfo}>
                ...
                {fromCoordinates && (
                  <div>
                    <input type="text" readOnly value={`Координаты: ${fromCoordinates.join(', ')}`} className={styles.addressField} />
            ...
            <div className={`${styles.inputGroup} ${styles.locationGroup}`}>
              ...
              <div className={styles.locationInfo}>
                ...
                {toCoordinates && (
                  <div>
                    <input type="text" readOnly value={`Координаты: ${toCoordinates.join(', ')}`} className={styles.addressField} />
          ...
          <div className={styles.section}>
            ...
            <div className={styles.imageUploadContainer} style={{ position: 'relative' }}>
              {previewUrls.length > 0 && mainImageIndex >= 0 && previewUrls[mainImageIndex] && (
                <div className={styles.mainImageContainer}>
                  <img src={previewUrls[mainImageIndex]} alt="Main preview" className={styles.mainImage} />
                  <div className={styles.mainImageCaption}>Это главное фото</div>
                </div>
              )}
              ...
              <div className={styles.thumbnailGrid}>
                {previewUrls.map((url, index) => (
                  <div key={index} className={styles.thumbnailWrapper}>
                    <img src={url} alt={`Thumbnail ${index}`} className={styles.thumbnail} />
                    <button
                      type="button"
                ...
                {previewUrls.length < 4 && (
                  <div className={styles.uploadPlaceholder} onClick={handleImageClick}>
                    {uploadProgress ? <span>{uploadProgress}%</span> : <span>+</span>}
                  </div>
                )}
              </div>
            ...
            <YMaps query={{ apikey: process.env.NEXT_PUBLIC_YANDEX_MAPS_API_KEY }}>
              <Map
                ...
                onClick={(e) => {
                  ...
                  if (isLocationFromOpen) setFromCoordinates(coords);
                  else setToCoordinates(coords);
                }}
                options={{
        ...
        <div className={styles.verificationPopup} style={errorPopupPosition}>
          <p style={{ color: '#ff3b30', fontWeight: 500 }}>
            {companyVerificationStatus === 'not_found'
              ...
              ? 'Чтобы сохранить поездку как физ. лицо, привяжите банковскую карту (Настройки → Мои карты).'
        ...
        <div className={styles.loadingOverlay} style={{ zIndex: 2000 }}>
          <div className={styles.verificationPopup} style={errorPopupPosition}>
            <p style={{ color: '#1e293b', fontWeight: 500 }}>
              Чтобы сохранить поездку, привяжите банковскую карту (Настройки → «Мои карты»)
  ...
  // Если компонент встраивается в DashboardMobile — не делаем свой скролл-контейнер (чтобы не было двойного скролла)
  if (!shouldRenderOwnScrollContainer) return content;

  // Если это отдельная страница /trips/edit-trip — делаем свой скролл-контейнер (как mainContentRef в DashboardMobile)
  return (
    <div
      ref={internalMainContentRef}
      style={{
        height: '100vh',
        overflowY: 'auto',

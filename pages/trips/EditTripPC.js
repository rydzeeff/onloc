...
export default function EditTripPC({ tripId }) {
  const router = useRouter();

  const handleBackToParticipants = () => {
    const id = tripId || (typeof router.query?.tripId === 'string' ? router.query.tripId : null);
  ...
  useEffect(() => {
    ...
    return () => content.removeEventListener('scroll', updatePositions);
      ...
      try {
        await new Promise((resolve, reject) => {
          const checkYmaps = () => {
            ...
            else setTimeout(checkYmaps, 100);
          };
          checkYmaps();
  ...
  return (
    <div className={styles.animatedWrapper}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <button type="button" onClick={handleBackToParticipants} className={styles.whatsBackBtn} disabled={loading}>
          ← Назад
        </button>
      </div>
      ...
      <div className={styles.formWrapper}>
        <form onSubmit={showTimezoneInput ? handleTimezoneSubmit : handleSubmit} className={styles.form}>
            ...
            <div className={styles.inputGroup}>
              ...
              {!hasValidCard && hasCompanyOk && (
                <p className={styles.hint}>
                  У вас нет привязанной карты — по умолчанию поездка сохраняется от лица компании. Чтобы переключиться на физ.
                  лицо, добавьте карту в «Настройки → Мои карты».
                </p>
              )}
            </div>
          </div>
          ...
          <div className={styles.section}>
            ...
            {/* ✅ алкоголь сразу после типа отдыха */}
            <div className={styles.inputGroup}>
              ...
              <div className={styles.thumbnailGrid}>
                ...
                {previewUrls.length < 4 && (
                  <div
                    ...
                    onClick={!loading ? handleImageUpload : undefined}
          ...
          {showTimezoneInput && (
            <div className={styles.section}>
              ...
              <div className={styles.inputGroup}>
                ...
                <select value={timezone || ''} onChange={handleTimezoneChange} disabled={loading} required>
          ...
          <div className={styles.loadingOverlay} style={{ zIndex: 2000 }}>
            <div className={styles.verificationPopup} style={errorPopupPosition}>
              <p style={{ color: '#1e293b', fontWeight: 500 }}>
                Чтобы сохранить поездку, привяжите банковскую карту (Настройки → «Мои карты») или зарегистрируйте компанию
                (Настройки → «Компании»).
              </p>
            </div>
          </div>

  ...
  // Если хочешь уведомление — пиши в общий чат поездки (trip_group), но не в ЛС.
    ...
    for (const p of paid) {
      await createTripAlert({
        ...
        body: `Поездка «${trip?.title || ''}» началась. Пожалуйста, подтвердите своё присутствие.`,
    ...
    try {
      ...
      for (const p of confirmedParticipants) {
        await createTripAlert({
          ...
          body: `Поездка «${trip?.title || ''}» завершена. Одобрите поездку или откройте спор в течение 12 часов.`,
          actorUserId: user?.id || null,
          metadata: { tripTitle: trip?.title || null },
          client: supabase,
        });

...
export const useEditTrip = (tripId) => {
  ...
  const calculateFees = (price) => {
    ...
    const platformFee = priceNum * (platformSettings.platformFeePercent / 100);
    const tbankFee = priceNum * (platformSettings.tbankFeePercent / 100);
    const netAmount = priceNum - platformFee - tbankFee;
    return { platformFee, tbankFee, netAmount };
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // fees
      const fees = calculateFees(geo.price);

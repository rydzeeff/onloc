...
export const useTripParticipantsActions = ({
  ...
  async function handleAccept(participantId) {
    try {
      ...
      // Берём user_id из БД (в кеше/RPC поле может отсутствовать)
      const { data: participantRow, error: participantRowErr } = await supabase
        .from('trip_participants')
        .select('user_id')
        .eq('id', participantId)
        .maybeSingle();
      if (participantRowErr) throw participantRowErr;

      const participantUserId = participantRow?.user_id || participant?.user_id;
      if (!participantUserId) {
        console.error('handleAccept: отсутствует user_id для участника', { participantId, participant });
        return setMessage('Пользователь подтверждён, но не удалось определить получателя уведомления');
      }
...
const text = isPaid
  ? `Вы были исключены из поездки "${trip?.title}" организатором. Средства возвращены в полном объёме.`
  : `Вы были исключены из поездки "${trip?.title}" организатором.`;
...
try {
  await createTripAlert({
    ...
    title: 'Вы исключены из поездки организатором',
    body: text,
    actorUserId: trip?.creator_id || null,

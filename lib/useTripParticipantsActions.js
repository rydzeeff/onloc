  ...
  async function handleAccept(participantId) {
    try {
      ...
      try {
        await sendMessage(
          participant.user_id,
          `Ваша заявка на поездку "${trip?.title}" подтверждена. Пожалуйста, оплатите ${trip?.price} руб. через форму оплаты.`
        );
      } catch (notifyErr) {
...
async function handleReject(participantId) {
  try {
    ...
    try {
      await sendMessage(
        participant.user_id,
        `Вас не приняли в поездку "${trip?.title}"`,
        false,
        null,
        { read: true, silent: true } // <- чтобы не было "непрочитанного хвоста"
      );
    } catch (notifyErr) {
...
await markTripPrivateChatsReadGlobally();
 ...
 // ВСПОМОГАТЕЛЬНАЯ: id общего чата поездки (trip_group)
  async function getTripGroupChatId() {
    try {
      ...
      if (error) {
        console.error('Не удалось получить trip_group чат:', error);
        return null;
      }

      return data?.id || null;
    } catch (e) {
      console.error('Ошибка при получении trip_group чата:', e);
      return null;
    }
  }
...
try {
  ...
  `Вас не приняли в поездку "${trip?.title}"`,
  false,
  memoizedTripId, // ✅ ВАЖНО
  { read: true, silent: true }
);
} catch (e) {
...
setConfirmModal({ open: false, action: null, participantId: null, confirmMessage: '' });
  ...
  async function approveAndPayout(participantId) {
   ...
   .maybeSingle();
      ...
      // 3) учёт комиссий (platform + tbank) из platformSettings
      const platformPercent = Number(platformSettings?.platformFeePercent || 0);
      const tbankPercent = Number(platformSettings?.tbankFeePercent || 0);
      const totalPercent = Math.max(0, platformPercent + tbankPercent);

      const netAmount = Number((grossAmount * (1 - totalPercent / 100)).toFixed(2)); // рубли, 2 знака
      if (!(netAmount > 0)) {
      ...
      try {
        await sendMessage(trip.creator_id, `Участник одобрил поездку «${trip.title}». Выплата ${netAmount} ₽ переведена.`);
      } catch (_) {}
...
try {
  ...
  // если сеть упала — не блокируем выход, идём дальше стандартно
    ...
    try {
      ...
      // 3) Сообщение в общий чат поездки (trip_group),
      //    чтобы у организатора была новая непрочитанка "X покинул поездку"
      const fullName = await getUserFullName(user?.id);
      try {
        const groupChatId = await getTripGroupChatId();
        if (groupChatId) {
await supabase.from('chat_messages').insert({
  chat_id: groupChatId,
  user_id: user?.id,
  content: `Пользователь ${fullName} покинул поездку "${trip?.title}"`,
});
        } else {
          console.warn(
            'trip_group чат не найден, системное сообщение о выходе не отправлено'
          );
        }
      } catch (msgErr) {
        console.error(
          'Ошибка отправки сообщения в trip_group при выходе из поездки:',
          msgErr
        );
      }

...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('[repeatTrip][restore] raw draft read', {
        hasFromLocation: Boolean(draft?.fromLocation),
        hasToLocation: Boolean(draft?.toLocation),
      ...
      // Адреса заполняем сразу, не дожидаясь загрузки изображений
      ...
      if (Array.isArray(fromCoords)) {
        ...
        console.warn('[repeatTrip][restore] fromLocation parse failed', draft?.fromLocation);
      }
      ...
      if (Array.isArray(toCoords)) {
        ...
        console.warn('[repeatTrip][restore] toLocation parse failed', draft?.toLocation);
      }

      console.log('[repeatTrip][restore] after parse', {
        fromCoords,
        toCoords,
        fromAddress: draft?.fromAddress || '',
        toAddress: draft?.toAddress || '',
...
const hasValid =
  ...
  const handleChange = (e) => {
    ...
    if (name === 'date') {
      ...
      if (value === '') {
        setTripData(prev => ({ ...prev, ageFrom: '' }));
        return;
      }

      const newAgeFrom = parseInt(value, 10);
      if (Number.isNaN(newAgeFrom)) return;

      setTripData(prev => {
        let newAgeTo = prev.ageTo;
        if (newAgeTo !== '' && newAgeFrom > newAgeTo) {
          newAgeTo = newAgeFrom;
      ...
      if (value === '') {
        setTripData(prev => ({ ...prev, ageTo: '' }));
        return;
      }

      const newAgeTo = parseInt(value, 10);
      if (Number.isNaN(newAgeTo)) return;

      setTripData(prev => {
        let newAgeFrom = prev.ageFrom;
        if (newAgeFrom !== '' && newAgeTo < newAgeFrom) {
          newAgeFrom = newAgeTo;
...
if (!tripData.isCompanyTrip) {
  ...
  if (!hasPrimaryValid) {
    ...
    const ageFrom = tripData.ageFrom === '' ? 18 : parseInt(tripData.ageFrom, 10);
    const ageTo = tripData.ageTo === '' ? 60 : parseInt(tripData.ageTo, 10);
    if (Number.isNaN(ageFrom) || Number.isNaN(ageTo)) {
      setTimeError('Укажите корректный возрастной диапазон.');
      setTimeout(() => setTimeError(null), 6000);
      setLoading(false);
      return;
    }

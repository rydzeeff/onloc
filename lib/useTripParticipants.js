...
export const useTripParticipants = (tripId) => {
  ...
  const isCheckinOpen = useMemo(() => {
    return (core.trip?.status || '').toLowerCase() === 'active_checkin' && isBeforePlannedStart;
  }, [core.trip?.status, isBeforePlannedStart]);
    ...
    for (const p of toExclude) {
      try {
        ...
        await core
          .sendMessage(
            p.user_id,
            `Вы исключены из поездки «${core.trip?.title}» из-за отсутствия оплаты к моменту старта.`
          )
          .catch(() => {});
      } catch (e) {
      ...
      try {
        ...
        if (isSelf && organizerId) {
          await core
            .sendMessage(
              organizerId,
              `Я подтвердил(а) присутствие по поездке «${title}».`
            )
            .catch((e) => {
              console.error(
                '[useTripParticipants] не удалось отправить ЛС о подтверждении присутствия:',
                e?.message || e
              );
            });
        }

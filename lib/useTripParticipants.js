...
async function handleStartTrip() {
  try {
    ...
    // 5) Оповестим каждого оплаченного участника о досрочном старте и необходимости чек-ина
    for (const p of paid) {
      await createTripAlert({
        userId: p.user_id,
        tripId: core.memoizedTripId,
        type: 'trip_started_checkin_required',
        title: 'Организатор начал поездку раньше времени',
        body: `Организатор начал поездку раньше запланированного времени. Подтвердите присутствие в поездке «${core.trip?.title || ''}», нажав кнопку «Подтвердить присутствие».`,
        actorUserId: core.trip?.creator_id || null,
        metadata: { tripTitle: core.trip?.title || null },
        client: supabase,
      });
    ...
    core.setMessage('Поездка переведена в режим чек-ина. Участники уведомлены о необходимости подтвердить присутствие.');
  } catch (e) {
  ...
  async function handleConfirmPresence(participantId) {
    try {
      ...
      try {
        ...
        if (isSelf && organizerId) {
          await createTripAlert({
            ...
            body: `«${await core.getUserFullName(core.user?.id)}» подтвердил присутствие по поездке «${title || ''}».`,
      ...
      const { data: paidRows, error: paidErr } = await supabase
        .from('trip_participants')
        .select('id, user_id, confirmed_start')
        .eq('trip_id', core.memoizedTripId)
        .eq('status', 'paid');
...
if (allConfirmed) {
        ...
        try {
          ...
          // Отправляем уведомление всем участникам поездки (кроме организатора — он уже уведомлён выше)
          const participantUserIds = (paidRows || [])
            .map((r) => r.user_id)
            .filter((uid) => uid && uid !== core.trip?.creator_id);
          ...
          for (const participantUserId of participantUserIds) {
            await createTripAlert({
              ...
              console.error('[useTripParticipants] не удалось уведомить участника о старте поездки:', err?.message || err);
            });
          }
        } catch (e) {
          console.error(

...
export const useTripParticipantsTrip = ({
    ...
    try {
      ...
      if (unpaidParticipants.length > 0) {
        ...
        for (const participant of unpaidParticipants) {
          ...
          await sendMessage(participant.user_id, `Вы исключены из поездки "${trip?.title}" за неоплату. Отзыв невозможен.`);
        }
      }
      ...
      for (const participant of paidParticipants) {
        await sendMessage(participant.user_id, `Поездка "${trip?.title}" началась! Пожалуйста, подтвердите своё присутствие в приложении.`);
      }
  ...
  async function confirmCancelTrip() {
    try {
      ...
      for (const participant of confirmedParticipants) {
        await sendMessage(participant.user_id, `Поездка "${trip?.title}" была отменена. Средства возвращены в полном объёме.`);
      }
      setConfirmModal({ open: false, action: null, participantId: null, confirmMessage: '' });
  ...
  async function handleFinishTrip() {
    try {
      ...
      const confirmedParticipants = participants.filter(p => p.status === 'confirmed' || p.status === 'paid');
      for (const participant of confirmedParticipants) {
        await sendMessage(participant.user_id, `Поездка "${trip?.title}" завершена. Одобрите или откройте спор (12ч).`);
      }
  ...
  async function handlePayoutOrganizer() {
    try {
      ...
      if (data.success) {
        ...
        const confirmedParticipants = participants.filter(p => p.status === 'confirmed' || p.status === 'paid');
        for (const participant of confirmedParticipants) {
          await sendMessage(participant.user_id, `Организатор получил выплату за поездку "${trip?.title}"`);
        }
  ...
  async function handleConfirmPresence(participantId) {
    try {
      ...
      await sendMessage(trip?.creator_id, `Участник ${fullName} подтвердил присутствие в поездке "${trip?.title}"`);
  ...
  async function handleApproveTrip(participantId, approved) {
    try {
      ...
      await fetchParticipants();
      const fullName = await getUserFullName(user.id);
      await sendMessage(trip?.creator_id, `Участник ${fullName} ${approved ? 'одобрил' : 'не одобрил'} поездку "${trip?.title}"`);
  ...
  async function handleOpenDispute(participantId, disputeReason) {
    try {
      ...
      // Уведомим организатора в личку (как и было), чтобы не ломать текущую логику уведомлений
      await sendMessage(
        trip?.creator_id,
        `Участник ${await getUserFullName(participant.user_id)} открыл спор по поездке "${trip?.title}". Проверьте вкладку «Поддержка».`,
        true,
        dispute.id
      );

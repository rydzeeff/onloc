    ...
    try {
      ...
      for (const participant of paidParticipants) {
        await notifyTripAlert({
          ...
          body: `Организатор начал поездку раньше запланированного времени. Подтвердите присутствие в поездке «${trip?.title || ''}», нажав кнопку «Подтвердить присутствие».`,
          actorUserId: trip?.creator_id || null,
          metadata: { tripTitle: trip?.title || null },
        });
      }
  ...
  async function handleConfirmPresence(participantId) {
    try {
      ...
      await notifyTripAlert({
        ...
        body: `«${fullName}» подтвердил присутствие по поездке «${trip?.title || ''}».`,

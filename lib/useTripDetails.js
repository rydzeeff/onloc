...
export const useTripDetails = ({ tripId, mapContainerRef } = {}) => {
  ...
  useEffect(() => {
    ...
    const tryInitMapWithRetry = (attempt = 0) => {
      if (cancelled || mapRef.current) return;
      try {
        initMap();
      } catch (error) {
        const isCtorRace = /not a constructor/i.test(String(error?.message || ''));
        if (isCtorRace && attempt < 3) {
          setTimeout(() => tryInitMapWithRetry(attempt + 1), 120);
          return;
        }
        throw error;
      }
    ...
    if (!trip || mapRef.current || !mapContainer) {
      if (trip && !mapRef.current && !mapContainer) {
        console.warn('[trip-details][map] skip init: map container is not ready', {
          effectiveId,
          hasMapContainerRef: !!mapContainerRef,
          hasMapContainerRefCurrent: !!mapContainerRef?.current,
    ...
    console.log('[trip-details][map] init requested', {
      effectiveId,
      pathname: router.pathname,
      source: mapContainerRef?.current ? 'ref' : 'id#map',
    });
    ...
    ensureYandexMapsReady()
      ...
      .catch((error) => {
        if (!cancelled) {
          console.error('[trip-details][map] Yandex Maps init failed:', {
            effectiveId,
            pathname: router.pathname,
  ...
  }, [trip, mapContainerRef, effectiveId, router.pathname]);
...
async function handleJoinTrip() {
  ...
  try {
    ...
    // 5.3) Оповестить организатора через trip_alerts (без системного сообщения в чат)
    ...
    await createTripAlert({
      ...
      tripId: trip.id,
      type: 'trip_join_request_created',
      title: 'Новая заявка в поездку',
      body: `Пользователь ${fullName} подал заявку на поездку «${trip.title || ''}».`,
      actorUserId: user.id,
      metadata: {
...
const imageUrls = useMemo(() => {
  ...
  function initMap() {
    ...
    const mapContainer = getMapContainerElement();
    if (!mapContainer) {
      console.error('[trip-details][map] initMap aborted: container not found', {
        effectiveId,
        pathname: router.pathname,
      });
      return;
...
console.log('[trip-details][map] initMap success', {
  effectiveId,
  pathname: router.pathname,
  fromCoords,
  toCoords,

...
if (reasonText && reasonText.trim()) {
      ...
      if (isOrganizerAuthor) {
        ...
        if (isBulk) {
          ...
          for (const p of participantsToReview) {
            ...
            // 🔔 ЛС участнику: организатор оставил отзыв
            if (typeof sendMessage === 'function') {
              try {
                await sendMessage(
                  p.user_id,
                  `Организатор оставил вам отзыв по поездке «${
                    trip?.title || ''
                  }».`
                );
              } catch (e) {
                console.error(
                  '[useTripDisputesReviews] не удалось отправить ЛС об отзыве (bulk):',
                  e?.message || e
                );
              }
            }
          ...
          // 🔔 ЛС участнику: личный отзыв от организатора
          if (typeof sendMessage === 'function') {
            try {
              await sendMessage(
                target.user_id,
              ...
              console.error(
                '[useTripDisputesReviews] не удалось отправить ЛС об отзыве (single):',
                e?.message || e
              );
            }
          }
        ...
        // 🔔 ЛС организатору: участник оставил отзыв
        if (trip?.creator_id && typeof sendMessage === 'function') {
          try {
            await sendMessage(
              trip.creator_id,
              `Я оставил(а) отзыв о поездке «${trip?.title || ''}».`
            );
          } catch (e) {
            console.error(
              '[useTripDisputesReviews] не удалось отправить ЛС организатору об отзыве:',
              e?.message || e
            );
          }
        }
  ...
  async function handleConfirmPresence(participantId) {
    try {
      ...
      await sendMessage(
        trip?.creator_id,
        `Участник ${fullName} подтвердил присутствие в поездке "${trip?.title}"`
      );
    } catch (error) {
  ...
  async function handleOpenDispute(participantRowId, disputeReason) {
    try {
      ...
      await sendMessage(
        trip?.creator_id,
        `Участник ${await getUserFullName(
          participant.user_id
        )} открыл спор по поездке "${trip?.title}". Проверьте вкладку «Поддержка».`,
        true,
        dispute.id
      );
